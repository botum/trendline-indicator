...
def generate_graph(pair, trades: pd.DataFrame, data: pd.DataFrame, args) -> tools.make_subplots:
    ...
    fig['layout']['xaxis1'].update(range=[data.date.min(),data.date.max()])
    fig['layout']['yaxis1'].update(range=[data.low.min()*0.98,data.high.max()*1.2])
    fig['layout']['xaxis2'].update(range=[data.date.min(),data.date.max()])
    fig['layout']['yaxis2'].update(range=[0,data.volume.max()])
    ...
    res_trend = go.Scatter(
        x=data.date,
        y=data.res_trend,
        name='Resistence',
        line={'color': 'red'},
    )
    sup_trend = go.Scatter(
        x=data.date,
        y=data.sup_trend,
        name='Support',
        fill="tonexty",
    ...
    fig.append_trace(res_trend, 1, 1)
    fig.append_trace(sup_trend, 1, 1)
    # print (data.res_trend)
